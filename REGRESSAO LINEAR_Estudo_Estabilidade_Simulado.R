...
grafico <- ggplot(dados, aes(x = Mes, y = Concentracao)) +
  ...
  annotate("text", x = shelf_life + 0.5, y = 91.5,
           label = paste0("Shelf life ≈ ", round(shelf_life, 1), " meses"),
           hjust = 0, size = 4, fontface = "italic", color = "black") +
  labs(
    title = "Estudo de Estabilidade: Tendência de Degradação",
    subtitle = "Modelo de Regressão Linear com Intervalo de Confiança (95%)",
    x = "Meses",
    y = "Concentração (%)"
  ) +
